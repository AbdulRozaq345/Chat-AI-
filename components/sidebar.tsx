...
export function SidebarDemo({ chatHistory, onSelectChat, onNewChat, onDeleteChat }: any) {
  const links = [
      {
        label: "New Chat",
        href: "#",
    ...
    >
          ...
          <div className="flex flex-1 flex-col overflow-x-hidden overflow-y-auto">
            ...
            {open && (
                <div className="px-2 mb-4">
                    <p className="text-[10px] text-neutral-200 uppercase mb-2 shrink-0">Search</p>
                    <input
                    value={searchTerm}
      ...
      {filteredCHistory.length > 0 ? (
        filteredCHistory.map((chat: any) => (
          /* KUNCINYA DI SINI: Gunakan group agar tombol hapus muncul saat hover */
          <div key={chat.id} className="group relative flex items-center pr-2">
            <div className="flex-1" onClick={() => onSelectChat(chat.id)}>
              <SidebarLink
                link={{
                  label: chat.title,
                  href: "#",
                  icon: <IconMessage2 className="h-5 w-5 shrink-0 rounded-full bg-neutral-600" />,
                }}
              />
            </div>

            {/* TOMBOL HAPUS (Muncul cuma kalau sidebar open DAN di-hover) */}
            {open && (
              <button
                onClick={(e) => {
                  e.stopPropagation(); // Mencegah chat terpilih saat mau dihapus
        ...
        open && <p className="text-sm text-neutral-500 px-2">No chat found.</p>
      )}
    </div>
  ...
  <div className="group relative">
    ...
    />
    {/* Tambahkan tombol Logout kecil saat Sidebar terbuka */}
    {open && (
      <button 
        onClick={() => signOut()} 
